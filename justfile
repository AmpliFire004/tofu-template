...
setup:
    ...
    remote_url=""; read -r -p "Enter git remote URL for origin (leave empty to skip): " remote_url; if [ -n "$remote_url" ]; then git remote add origin "$remote_url"; echo "Set git remote 'origin' to $remote_url"; else echo "Skipped setting git remote 'origin'."; fi
